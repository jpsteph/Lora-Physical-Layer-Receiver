...
def lora_decode_header(symbols_message, SF):
    rdd_hdr         = 4 
    ppm_hdr         = SF - 2 
    ...
    if((CR_pld > 4) or (CR_pld < 1)):
        print('Code Rate Invalid')
        return 

    CRC_pld         = (mt.floor((int(symbols_hdr_fec[1])>>4)) % 2) 
    payloadlength      = symbols_hdr_fec[0] + CRC_pld*2 

    return codingrate, payloadlength

# LoRa_Decode_Full decodes full payload 
#
